...
# Это означает, что когда тип значения `object`, проверка типов отклонит почти все операции над ним, а присвоение его
# переменной (или использование в качестве возвращаемого значения) более специализированного типа является ошибкой
# типа.

# Тип `object` - это еще один тип, который может иметь экземпляр произвольного типа в качестве значения. В отличие от
# `Any`, `object` обычный статический тип (похожий на Object в Java), и для значений объекта принимаются только
# операции, допустимые для всех типов.

# `object` может быть приведен к более конкретному типу, в то время как Anyна самом деле означает, что все допустимо,
